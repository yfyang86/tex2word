% Taken from https://www.overleaf.com/project/5fbb3172ffac690c4124ca8a, which itself is based on 
% a template by Drew Ulick: https://www.overleaf.com/articles/130-cheat-sheet/ntwtkmpxmgrp
...
\node [mybox] (box){%
    ...
    $\bullet \forall$ a $\in$ G, $a^{-1}$ is uniquely determined\\
    $\bullet (a^{-1})^{-1}$ = a $\forall$ a $\in$ G\\
    $\bullet (a * b)^{-1}$ = $(b^{-1})\ *\ (a^{-1})$\\
    $\bullet$ for any $a_1 , a_2, \dots , a_n$ $\in$ G the value of $a_1 * a_2 * \cdots *a_n$ is independent of how the expression is bracketed
    \end{minipage}
};
%------------ Mixing Header ---------------------
